...
function generarCotizacion(texto) {
  const config = { taOneWay: 30700, taRoundTrip: 49750 };
  ...
  return {
    ...
    total,
    pasajeros,
    mensaje: generarMensaje(segmentos, tipo, equipaje, totalConTA, pasajeros),
  };
}
...
function ejecutarPrueba(numero, nombre, input) {
  ...
  console.log('Total con TA:', formatearCOP(resultado.totalConTA || resultado.total));
  console.log('');
  console.log('MENSAJE:');
